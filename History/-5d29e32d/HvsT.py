...
def plot_scatter_gp(data, x, y, hue=None, style=None, col=None, row=None, xscale='log', yscale='log', output_path=None):
    ...
    return g
# %%
def plot_decile_gp(df_gp, y_name, y_pred_names, col_gp, row=None, col=None, sharey=True, debug=False, plot_type='line', yscale='log', output_path=None, col_wrap=None, show_predict=True, reflines: list[tuple] = None):
    ...
    if plot_type=='line':
        ...
        #     # boxplot
        #     g.map_dataframe(
        #         sns.boxplot, x=f'decile_{y_pred_name}', y='total_bill', 
        #         # labels=f"total_bill_{y_pred_name}", 
        #         color=next(colors), 
        #         whis=3
        #         )
        ...
        #     if show_predict: 
        #         g.map_dataframe(
        #             sns.boxplot, x=f'decile_{y_pred_name}', y=y_pred_name, 
        #             # labels=y_pred_name, 
        #             color=next(colors), whis=3
        #             )

        # elif plot_type=='violin':
        #     # violin plot
        #     g.map_dataframe(
        #         sns.violinplot, x=f'decile_{y_pred_name}', y='total_bill', 
        #         label=f"total_bill_{y_pred_name}", 
        #         color=next(colors), 
        #         scale='width'
        #         )
        ...
        #     if show_predict: 
        #         g.map_dataframe(
        #             sns.violinplot, x=f'decile_{y_pred_name}', y=y_pred_name, 
        #             label=y_pred_name, 
        #             color=next(colors), scale='width'
        #             )

    if reflines:
        for axis, val, label in reflines:
            if axis == 'x':
                g.refline(x=val, label=label)
            else:
                g.refline(y=val, label=label)
...
# %%
"""
# this version is for plotting the decile plot of a SINGLE model only!
def plot_decile_gp(df_gp, y_name, y_pred_name, col_gp, row=None, col=None, sharey=True, debug=False, plot_type='line', yscale='log', output_path=None, col_wrap=None):
    ...
    feature=[x for x in [col, row] if x] # get the names given to `row` and `col`, but excldue it if it is `None`

    # ------------------------ add a new column to the df ------------------------ #
    if feature: # if col or row:
        df_decile['decile'] = df_decile.groupby(feature)[y_pred_name].transform(lambda x: pd.qcut(x, q=10, labels=range(1,11)))
    else: # both col and row are None
        df_decile['decile'] = pd.qcut(df_decile[y_pred_name], q=10, labels=range(1,11))
    # ---------------------------------------------------------------------------- #
    ...
    if debug:
        print(f"overall:\n{df_decile.decile.value_counts(normalize=True)}")
        if feature: 
            for name, x in df_decile.groupby(feature):
                print(f"decile {name}")
                print(x['decile'].value_counts(normalize=True))
    # ---------------------------------------------------------------------------- #
...
# df_gp
# play = df_gp.copy()
# play['decile']=pd.qcut(df_gp.y_pred, q=10, labels=range(1,11))
# play
# # play_melt = _decile.melt(id_vars=col_gp+['decile'], var_name='output', value_name=y)
# sns.lineplot(
#     data=play, x='decile', y='y_pred', ci=None
# )
# sns.lineplot(
#     data=play, x='decile', y='y_true', ci=None
# )
# %%
