...
def add_prefix_suffix(path, ext='', prefix='', suffix='', ignore_hidden=True, recursive=False, timestamp=False):
    ...
    for file in files:
        ...
        if timestamp == 'prefix':
            new_name = '_'.join([get_timestamp(), new_name])
        elif timestamp == 'suffix':
            new_name = '_'.join([new_name, get_timestamp()])
...
parser.add_argument('-t', '--timestamp', action='store', help='add timestamp as prefix or suffix', default=False)
...
if args['path']: 
    # print("enter")
    add_prefix_suffix(args['path'], prefix=args['prefix'], suffix=args['suffix'], ignore_hidden=args['ignore_hidden'], recursive=args['recursive'])