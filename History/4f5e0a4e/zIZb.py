...
#     return df_output
# %%
def cal_metrics(y_true, y_pred: dict, metrics, precision=6, index_name='metric', columns_name='model'):
    pd.DataFrame(y_pred)
    res = dict()
    ...
    return pd.DataFrame(res).T.round(precision).rename_axis(index=index_name, columns=columns_name)
