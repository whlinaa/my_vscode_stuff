...
\begin{questions}
  ...
  \begin{enumerate}
    ...
    % \item Find $\int_{-\infty}^{\infty}P(X>x)dx$
    % \item $\int_{2}^{3}$
  \end{enumerate}
...
\begin{enumerate}
  ...
  \item Find the probability that $X$ is greater than $2.5 \mathrm{~kg}$.
  \item A butcher buys the bags and can use them in one of two processes:
  \begin{itemize}
    \item The complete contents (including inedibles) are ground up and made into standard sausages. If the bag contains more than $2.5 \mathrm{~kg}$ of inedibles the resulting sausages are unsaleable and a loss of $\$ 44$ is made on the bag, otherwise a profit of $\$ 14$ is made on each bag.
    \item The inedibles are extracted and the remainder made into premium sausages. The profit on this process is $\$(40-16 X)$ per bag.
  \end{itemize}
