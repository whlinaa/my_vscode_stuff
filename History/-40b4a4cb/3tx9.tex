...
\part Does there exists a pair of positive real numbers $a$ and $b$ such that $\sqrt{a}+\sqrt{b} = \sqrt{a+b}$? If so, find them. If not, prove such a pair does not exist.
% \fillwithlines{4in}
\part Prove or disprove that $\sqrt{2}+\sqrt{37460123}$ is a rational number. Do not assume that $\sqrt{37460123}$ is irrational or rational. You may, however, take for granted that $\sqrt{2}$ is irrational, since this has been proved in class. State clearly whether you think the number is rational or not in your answer.
